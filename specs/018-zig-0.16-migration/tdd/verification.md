...
2. `fs.zig` RealFs.realpath is now a manual readLink loop (0.16 has no
   `realpath`); symlink chains beyond 8 hops fall back to the lexical path.
3. `readLine` in repl.zig reimplements line assembly over raw Reader primitives
   (`takeDelimiterExclusive` spins on fixed readers in 0.16.0).
4. Dual-toolchain (0.15+0.16) compatibility was explicitly NOT attempted — the
   floor bumps to 0.16.0 per the issue's sanctioned fallback.
5. Local verification used CI's exact command on macOS; the Linux runner is the
